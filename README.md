...
`PATH`, you can also launch the app as `ckan-linux`. The local installer does
not install or replace the `ckan` command; it only installs the LinuxGUI
launcher.
...
This local install stays separate from any system `ckan` command. The
`/usr/bin/ckan` wrapper behavior described above applies to Debian/package
installs built from this fork, not to the default `~/.local` installer.
...
- Debian package integration that routes graphical no-argument `ckan` launches to
  ...
  intact.
- Optional fast catalog browsing through Rust-generated sidecar indexes from
...
The LinuxGUI mod browser includes a sidecar reader for prebuilt catalog/search
indexes generated by [`appaKappaK/ckan-meta-rs`](https://github.com/appaKappaK/ckan-meta-rs).
That separate Rust project parses CKAN metadata archives and writes the
`catalog-index` JSON consumed by this repo.

When configured, the sidecar is the fast browse/search path for the LinuxGUI
catalog. CKAN core remains authoritative for metadata details, installs,
dependency resolution, compatibility decisions, and registry writes. If a
sidecar index file is missing, invalid, or not configured, the LinuxGUI falls
back to the normal CKAN metadata loader.

See the [LinuxGUI sidecar workflow](LinuxGUI/README.md#rust-catalog-sidecar)
for the `CKAN_CATALOG_INDEX_PATH` development setup and catalog load timing
notes.
...
packaging, development, `ckan-meta-rs` catalog sidecar, logging, and visual-test
workflow.
