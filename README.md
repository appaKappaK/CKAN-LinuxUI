...
- Debian packaging that routes graphical no-argument `ckan` launches to
  ...
  intact.
- Fast catalog browsing through the Rust-generated sidecar index from
...
The sidecar is the fast browse/search path for the LinuxGUI catalog. CKAN core
remains authoritative for metadata details, installs, dependency resolution,
compatibility decisions, and registry writes. If a sidecar index file is missing,
invalid, or not configured in a development run, the LinuxGUI falls back to the
